  ...
  useEffect(() => {
    dispatch(fetchOrdersByUserId(currentUser.userID)).then(async (response) => {
      const ordersWithItems = await Promise.all(
        response.payload.map(async (order, index) => {
          const orderItems = await orderItemApi.getOrderItemsByOrderId(
            order.orderID,
          );
      ...
      {orders.map((order) => (
        <div className="my-orders-div" key={order.orderID}>
          ...
          <div className="my-orders-about space-between">
            <ul>
              <li>
                <p className="grey txt">Order #{order.orderID}</p>
              </li>
              <li>
                <p>Placed {formatDateTime(order.dateTime)}</p>
              </li>
            </ul>
            <ul>
              {order.status === 0 && (
                <li>
                  <a className="red txt">Cancel</a>
                </li>
              )}
              <li>{getStatusString(order.status)}</li>
            </ul>
          </div>
          <div>
